...
import type { ChannelDetailsPaneContent, ChannelDetailsTab } from "../../../lib/store/slices/types";
...
export default function ChannelDetails(props: { pane: Pane<ChannelDetailsPaneContent> }) {
  const channelId = () => props.pane.content.channelId;
  const [tab, setTab] = createSignal<ChannelDetailsTab>(props.pane.content.tab ?? "about");
  ...
  const [details, { refetch }] = createResource(channelId, loadChannelDetails);

  createEffect(
    on(
      () => [channelId(), props.pane.content.tab] as const,
      ([, requestedTab]) => setTab(requestedTab ?? "about"),
    ),
  );
  ...
  const saveTopic = async () => {
    ...
    if (previous === undefined || v === previous || savingTopic()) return;
  ...
  const savePurpose = async () => {
    ...
    if (previous === undefined || v === previous || savingPurpose()) return;
    await saveEditableField({
      next: v,
  ...
  const [copiedKey, copy] = createCopyFeedback(1200, () => {
    actionFeedback.flash(channelId(), "Couldn't copy to the clipboard.", "error");
  });
  ...
  return (
    <Show
      fallback={
        <div class="channel-details-card flex-col" data-pane={props.pane.id}>
          <PanelHeader onClose={() => closeTile(props.pane.id)} title="Channel details" />
          <Show
            fallback={
              <div class="channel-details-load-error flex-center flex-col">
                <span>Couldn't load channel details.</span>
                <InlineFeedback feedback={actionFeedback.get(channelId())} priority={2} />
            ...
            <Show when={tab() === "about"}>
              ...
              <div class="channel-details-copy-list flex-col">
                ...
                >
                  ...
                  <span class="channel-details-copy-value truncate">Copy link to channel</span>
                ...
                >
                  ...
                  <span class="channel-details-copy-value truncate">Channel ID: {d().id}</span>
