import { extractSlackSession, type StoredAccount } from "@slock/types";
...
export function parseSlackCurl(raw: string): Omit<StoredAccount, "id" | "name" | "avatarUrl"> {
  const text = raw
    .trim()
  ...
  return { domain, route, slackSession, token };
}
