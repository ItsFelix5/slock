...
- **Saved searches / search history**: MessageSearchView's filters are solid
  but throwaway per-session; persisting the last few searches (or letting you
  name and save one) would make repeated lookups ("my open PRs channel",
  "unresolved bugs") much faster than Slack's own search.
