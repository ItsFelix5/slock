...
function RichTextLeaf(props: { el: RichTextInlineElement }) {
  ...
  switch (el.type) {
    ...
    case 'emoji':
      return el.unicode ? <span class="emoji">{el.unicode}</span> : <EmojiText text={`:${el.name}:`} />;
    case 'user':
            ...
            case 'rich_text_list':
              return (
                <Show
                  ...
                  fallback={
                    <ul class="bk-rt-list" style={{ 'padding-left': `${16 + (sub.indent ?? 0) * 20}px` }}>
                      <For each={sub.elements}>{(item) => <li><RichTextInline elements={item.elements} /></li>}</For>
                    </ul>
                  }
                >
                  <ol class="bk-rt-list" style={{ 'padding-left': `${16 + (sub.indent ?? 0) * 20}px` }}>
                    <For each={sub.elements}>{(item) => <li><RichTextInline elements={item.elements} /></li>}</For>
                  </ol>
                </Show>
              );
