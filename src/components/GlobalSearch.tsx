...
import { fetchBrowsableChannels } from '../slackApi';
import type { BrowsableChannel, Channel, User } from '../types';
...
// Rank exact/prefix matches above mid-string ones so e.g. querying "general"
// surfaces #general before #off-topic-general-chat. Only used as a tiebreaker
// within a group that has no real backend ranking of its own (see below).
...
export default function GlobalSearch(props: { onClose: () => void }) {
  ...
  // One flat, ranked list instead of separate headed sections — channels/people
  // jump results ranked together by frecency (frequency + recency of visits),
  // the same signal the real client's quick switcher uses its local jump-target
  // database for, with match quality and name as tiebreakers.
  ...
  const rows = createMemo<Row[]>(() => {
    if (!hasQuery()) return [];
    const q = query().trim().toLowerCase();
    type Ranked = { name: string; score: number; row: Row };
    const ranked: Ranked[] = [
      ...channelResults().map((c): Ranked => ({ name: c.name, score: frecencyScore(c.id), row: { kind: 'channel', data: c } })),
      ...peopleResults().map((u): Ranked => ({ name: u.name, score: frecencyScore(u.id), row: { kind: 'person', data: u } })),
    ];
    ranked.sort((a, b) => {
      const scoreDiff = b.score - a.score;
      if (scoreDiff !== 0) return scoreDiff;
      const rankDiff = matchRank(a.name, q) - matchRank(b.name, q);
